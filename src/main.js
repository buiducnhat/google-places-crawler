...
async function crawlDetailsExtra({ query, page, urls, index }) {
  const urlPattern = /!1s(?<id>[^!]+).+!3d(?<lat>[^!]+)!4d(?<lon>[^!]+)/gm;
  const result = [];
  let hasError = false;
  let retry = 0;
...
async function crawlDetailsDivide({ query, browser }) {
  blockResourcesPlugin.blockedTypes.delete('image');
  ...
  const subUrls = [];
  const pages = [];
  const l = Math.ceil(totalUrls.length / 10);
  for (let i = 0; i < 10; i++) {
...
async function main() {
  ...
  switch (action) {
    ...
    case 'crawl-direct':
      ...
      break;
    case 'crawl-details-divide':
      await crawlDetailsDivide({ query, browser });
      break;
    default:
